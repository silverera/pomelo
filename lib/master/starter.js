 ...
 */
 starter.runServers = function(app) {
  ...
  switch(condition) {
    ...
    case Constants.RESERVED.ALL_EXCLUDE_MASTER:
...
var spawnProcess = function(command, host, options, cb) {
  ...
  child.on('exit', function (code, signal) {
    if(code !== 0) {
      logger.warn('child process exit with error, error code: %s, executed command: %s, signal: %s, options:%s', code,  command, signal, options);
    }
