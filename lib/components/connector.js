...
pro.send = function (reqId, route, msg, recvs, opts, cb) {
    if (route!='onCharStateChange' && route!='replicator.replicationHandler.hearBeat')
        logger.trace('[%s] send message reqId: %s, route: %s, msg: %j, receivers: %j, opts: %j', this.app.serverId, reqId, route, msg, recvs, opts);
...
var getSession = function (self, socket) {
    ...
    session.set('packetCache', []);
    session.set('nextReqID', 1);
    session.set('nextPushMsgID', 1);
...
var handleMessage = function (self, session, msg) {
    if (msg.route.indexOf('uploadCharState')<0 && msg.route !='replicator.replicationHandler.hearBeat')
        logger.trace('[%s] handleMessage session id: %s, msg: %j', self.app.serverId, session.id, msg);
    ...
    if (msg.id){
        var expect = session.get('nextReqID');
        if(msg.id < expect) {
            logger.warn('requestID(%d) invalid, potential cheating. route:%s,ip:%s,account:%s,expectID:%d',
                msg.id, msg.route, session.__socket__.remoteAddress.ip, session.get('account'), expect);
            return;
        }
        session.set('nextReqID', expect + 1);
    }
    ...
    self.server.globalHandle(msg, session.toFrontendSession(), function (err, resp, opts) {
        ...
        if (resp) {
            if (resp.__pushMsgs) {
                __pushMsgs = resp.__pushMsgs;
                delete resp.__pushMsgs;
            }
        ...
        if (__pushMsgs) {
            for (var id in __pushMsgs) {
                var pushMsg = __pushMsgs[id];
                self.send(null, pushMsg.route, pushMsg.resp, [session.id], null, function () {
                });
            }
        }
